...
export class ParamMessage implements OnInit {
  ...
  ["kaimosudu1", "0x20140017"],
  ["kaimosudu2", "0x20140016"],
  ["kaimosudu3", "0x20140015"],
  ["kaimosudu4", "0x20140014"],
  ["kaimosudu5", "0x20140013"],
  ...
  getParamData() {
    this.http.get(Global.domain + 'api/deviceInfo.action?mid=' + this.current_mid).subscribe((res) => {
      ...
      {
        this.normal = false;
        this.http.get(Global.domain + 'api/remoteparamGet.action?mid=' + this.current_mid).subscribe((res) => {
          //console.log(res.json());
            ...
            for (var j = 0; j < this.AllEaddress.length; j++) {
              if (this.AllEaddress[j][1] == data[i].addr) {
                if (this.AllEaddress[j][0].indexOf('Code') == -1) {
                  let _value = typeof data[i].value  =="number" ? Number(data[i].value).toFixed(1) + '' : '';
            ...
            for (var j = 0; j < this.addressArray.length; j++) {
              if (this.addressArray[j][1] == data[i].addr) {
                if (this.addressArray[j][0].indexOf('Code') == -1) {
                  this.MData[this.addressArray[j][0]] = data[i].value;
                } else {
