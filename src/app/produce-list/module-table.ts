...
export class ModuleTable implements OnInit {
  ...
  changeClient(num) {
    ...
    swal({
      ...
      if (result.value) {
        ...
        var name = <HTMLInputElement>document.getElementById('name');
        var status = <HTMLInputElement>document.getElementById('status');
        this.http.get(Global.domain + 'api/apisaveClient.action?client.id=' + this.page_data[num].clientid + '&client.clientId=' + clientId.value + '&client.clientName=' + name.value + '&client.status=' + status.value).subscribe((res: Response) => {
          if (res.json().code == 200) {
            this.ClientEvent.emit("ok");
          }
        })
  ...
  deleteClient(num) {
    swal({
      ...
      if (result.value) {
        this.http.get(Global.domain + 'api/apidelClient.action?clientId=' + this.page_data[num].clientid).subscribe((res: Response) => {
          if (res.json().code == 200) {
            this.ClientEvent.emit("ok");
          }
        })
  ...
  resetPsd(num) {
    swal({
      ...
      if (result.value) {
        this.http.get(this.url + 'apiresetPassword.action?userId=' + this.page_data[num].id).subscribe((res: Response) => {
    ...
    <ul class="EmployeeCha">
      ...
      swal({
        ...
        if (result.value) {
          ...
          var Mtype = <HTMLInputElement>document.getElementById('status');
  ...
  editMachineBcck(num, data_model, data_area, data_company) { //add by yangjie 20180427 
    ...
    <ul class="EmployeeCha">
      ...
      <li><label>备注信息：</label><input type="text" id="remarks" placeholder="备注信息" ${_machine.remarks}/></li>
    </ul>`;
      /*这里的select应该从数据获取，而不是这里写死*/
      swal({
        title: "编辑机器",
