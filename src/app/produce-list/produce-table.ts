...
export class ProduceTable implements OnInit {
  ...
  getMachineCount(callback):void{
    this.http.request(Global.domain+'api/apideviceList.action')
    .subscribe((res:Response)=>{
      ...
      {
        if(res.json().obj[i].companyid==this.companyid || !this.companyid) //add by yangjie 20180426 || !this.companyid
        {
