...
export class SearchMachineidComponent implements OnInit {
  ...
  ngOnInit() {
    this.getMachineID(() => {
      ...
      if (current_monitorid != undefined) {
        this.http.get(Global.domain + 'api/deviceInfo.action?mid=' + current_monitorid).subscribe((res: Response) => {
          if (res.json().obj.monitorid) { //add by yangjie 20180426 增加判断 正式和测试机mid不匹配缓存会影响
  ...
  ngAfterContentInit() {
    ...
    if (this.Mapmid != undefined) {
      ...
      this.http.get(Global.domain + 'api/deviceInfo.action?mid=' + this.Mapmid).subscribe((res: Response) => {
        var data = res.json().obj;
        this.detail_Machine.id = data.sn;
        this.detail_Machine.name = data.name;
        this.detail_Machine.out_date = data.ddate != null ? data.ddate.substring(0, 10) : "暂无";
